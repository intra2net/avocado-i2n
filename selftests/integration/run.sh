...
test $(ls -A1q "$test_results/latest/test-results" | grep manage.unchanged.vms.vm1 | wc -l) == 5 || (echo "Incorrect number of vm1 state retrieval tests" && exit 1)
test $(ls -A1q "$test_results/latest/test-results" | grep manage.unchanged.vms.vm2 | wc -l) == 5 || (echo "Incorrect number of vm2 state retrieval tests" && exit 1)

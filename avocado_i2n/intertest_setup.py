...
def job_augmented_graph(graph):
    """
    Run a test node from a single params parser.

    :param graph: graph to augment with a job resource
    :type graph: :py:class:`CartesianGraph`

    This includes producing a job for each call or
    a step beyond test running as full job running.
    """
    with job.Job(graph.args) as job_instance:
        ...
        try:
            ...
            graph.__init__(job=job_instance, result=job_instance.result)
            yield graph
...
def graphfull(args, run_params, tag=""):
    ...
    for vm_name in run_params.objects("vms"):
        ...
        create_graph = CartesianGraph(args, {})
        # overwrite any existing test objects
        vm_params["force_create"] = "yes"
        with job_augmented_graph(create_graph):
...
def graphupdate(args, run_params, tag=""):
    ...
    for vm_name in run_params.objects("vms"):
        ...
        remove_graph = CartesianGraph(args, {})
        with job_augmented_graph(remove_graph):
        ...
        with job_augmented_graph(update_graph):
            ...
            update_graph.parse_object_trees(args.param_str, param.re_str(vm_params.get("to_state", "customize_vm")),
            ...
            if vm_params.get("from_state", "install") != "root":
                reuse_graph = CartesianGraph(args, {})
                reuse_graph.parse_object_trees(args.param_str, param.re_str(vm_params.get("from_state", "install")),
                                               {vm_name: args.vm_strs[vm_name]}, prefix=tag,
...
def run(args, run_params, tag=""):
    ...
    trees = CartesianGraph(args, {})

    with job_augmented_graph(trees):
        # essentially we imitate the auto plugin to make the tool plugin a superset
        trees.args.test_runner = CartesianGraph
        job = trees.job
        job.args.sysinfo = 'on'
...
def list(args, run_params, tag=""):
    ...
    trees.parse_object_trees(args.param_str, args.tests_str, args.vm_strs, prefix=prefix)
    trees.visualize(data_dir.get_base_dir())
...
def windows(args, run_params, tag=""):
    ...
    vms = graph.parse_objects(args.vm_strs, run_params.get("vms", ""))
    for vm in vms:
...
def check(args, run_params, tag=""):
    ...
    tests = graph.parse_object_nodes(setup_str, args.vm_strs,
                                     object_names=run_params["vms"],
                                     objectless=True, prefix=tag)
    with job_augmented_graph(graph):
        for test in tests:
            graph.run_test_node(TestNode(tag, test.parser, []))
...
def get(args, run_params, tag=""):
    ...
    graph = CartesianGraph(args, {})
    for vm_name in run_params.objects("vms"):
        parser = param.update_parser(graph.parse_objects(args.vm_strs, vm_name)[0].parser,

...
class CartesianLoader(Resolver):
    ...
    def parse_flat_nodes(self, param_dict=None, nodes_str=""):
        """
        ...
        for d in early_config.get_parser().get_dicts():
    ...
    def parse_and_get_objects_for_node(self, test_node: TestNode, test_graph: TestGraph, params: dict[str, str] = None) -> list[TestObject]:
        """
        ...
        :param test_graph: test graph of already parsed nodes and objects we could reuse from
        :param params: runtime parameters used for extra customization
        """
        d, graph = test_node.params, test_graph
        ...
        for combination in itertools.product(*get_vms.values()):
            ...
            if len(reused_nets) == 1:
                ...
                net = self.parse_object_from_objects("net1", "nets", combination, params=params, verbose=False)
                parse_nets["net1"] += [net]
                graph.objects += [net]
            else:
    ...
    def parse_and_get_nodes_for_node_and_object(self, test_node: TestNode, test_object: TestObject, test_graph: TestGraph,
    ...
    def parse_nodes(self, test_graph, param_dict=None, nodes_str="", prefix="", verbose=False):
        ...
        for i, node in enumerate(self.parse_flat_nodes(param_dict, nodes_str)):
            ...
            try:
                test_nets = self.parse_and_get_objects_for_node(node, test_graph, params=param_dict)
            except ValueError:
    ...
    def parse_object_trees(self, param_dict=None, nodes_str="", object_strs=None,
        ...
        while len(unresolved) > 0:
            ...
            for test_object in test_node.objects:
                ...
                get_parents, parse_parents = self.parse_and_get_nodes_for_node_and_object(test_node, test_object, graph, param_dict)
